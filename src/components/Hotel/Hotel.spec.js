...
describe("Hotel", () => {
          ...
          hotels: [
            {
              ...
              hotelStaticContent: {
                ...
                mainImage: {
                  category: "EXTERIOR",
                  url: "http://example-moya.jpg",
                  source: "SOURCE1",
                },
                name: "Moya Chicago Hotel & Suites Magnificent Mile",
  ...
  it("renders the Hotel component", () => {
    ...
    expect(wrapper.find(".hotel").exists()).toBe(true);
  ...
  it("should load list of hotels", async () => {
    let wrapper;
    await act(async () => {
      await axios.get.mockImplementationOnce(() =>
        Promise.resolve(responseResolve)
      );
      wrapper = mount(<Hotel />);
    });
